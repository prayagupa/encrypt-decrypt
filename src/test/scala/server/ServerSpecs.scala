package server

import javax.xml.bind.DatatypeConverter

import client.Client
import org.scalatest.{FunSuite, Matchers}

class ServerSpecs extends FunSuite with Matchers {

  test("given private key, decrypts the bytes") {
    val format = "der"

    Client.enc("src/main/resources/keypair_" + format.toUpperCase + "/public_key." + format, "my data").foreach(c => {
      Server.decrypt("src/main/resources/keypair_" + format.toUpperCase + "/private_key." + format, c).map(s => {
        new String(s) shouldBe "my data"
      })
    })

  }

  test("given private key, decrypts hex") {
    val value1 = ""
    println(value1.getBytes().length)
    val hexBytes = DatatypeConverter.parseHexBinary(value1)

    val format = "der"
    Server.decrypt("src/main/resources/keypair_" + format.toUpperCase + "/private_key." + format, hexBytes).map(s => {
      new String(s) shouldBe "data to encrypt"
    })
  }
}
